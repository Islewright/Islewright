...
struct ClientInfo
{
    ...
    ClientInfo(SOCKET clientSocket, const SOCKADDR_IN& clientAddr) : m_socket(clientSocket), m_clientAddr(clientAddr)
    {
        m_recvBuffer = new char[m_bufferSize + 1];
        memset(&m_recvBuffer, 0x00, m_bufferSize + 1);
        m_sendBuffer = new char[m_bufferSize + 1];
        memset(&m_sendBuffer, 0x00, m_bufferSize + 1);
    }
